...
contract PayvergePaymentsSecurityTest is Test {
    ...
    function testSuccessfulPaymentFlow() public {
        ...
        uint256 contractBalanceBefore = usdc.balanceOf(address(payverge));
        ...
        // Check that the full payment amount is now in the contract
        // (business amount is claimable, platform fee stays in contract)
        uint256 contractBalanceAfter = usdc.balanceOf(address(payverge));
        assertEq(contractBalanceAfter - contractBalanceBefore, BILL_AMOUNT);
        ...
        (uint256 claimableAmount,) = payverge.getClaimableAmounts(businessOwner, businessOwner);
        assertEq(claimableAmount, expectedBusinessAmount);
    }
    ...
    function testPlatformFeeEdgeCases() public {
        ...
        // Since no profit split contract is set, fees remain in the contract
